...
class PerlinNoise(GeneratorModule):
    def init(self):
    ...
    def apply(self, map_width, map_height, settings, inputs, rng):
        ...
        for i in range(map_height):
            for j in range(map_width):
                # remaps native -1 to +1 to 0 - noise_height
                map_out[i][j] = (pnoise2(i * scale + rndx, j * scale + rndy) + 1) * half_noise_height
        return {"out": map_out}

class SimplexNoise(GeneratorModule):
    def init(self):
    ...
    def apply(self, map_width, map_height, settings, inputs, rng):
        ...
        for i in range(map_height):
            for j in range(map_width):
                # remaps native -1 to +1 to 0 - noise_height
                result = snoise2(i * scale + rndx, j * scale + rndy)
                map_out[i][j] = (result + 1) * half_noise_height
